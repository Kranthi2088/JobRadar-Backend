...
// Ensure local `.env` wins over inherited shell env (turbo/dev can inject blanks).
config({ path: resolve(root, ".env"), override: true });
config({ path: resolve(root, ".env.local"), override: true });

const prisma = new PrismaClient();
// Debug helper for local env wiring (do not log the secret itself).
console.log("[api env]", {
...
async function requireUser(req: any) {
  ...
  const jwtReq = { headers: { cookie: cookieHeader }, cookies } as any;
